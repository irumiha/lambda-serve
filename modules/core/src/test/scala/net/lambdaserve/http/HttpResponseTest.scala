...
class HttpResponseTest extends FunSuite:
  ...
  test("Response addHeader adds single header"):
    val response = HttpResponse.Ok("test").addHeader("X-Custom", "value")
  ...
  test("Response can have content length"):
    val response = HttpResponse.Ok("test").copy(length = Some(100L))
